...
def create_hashed_password(plain_text_password) -> str:
    plain_text_password = plain_text_password.encode('utf8')
...
def new_user(email, password)->str:
    ...
    if users is None:
        ref.push().set({"id": 0, "email": email, "password": create_hashed_password(password), "language" : "en"})
        return "successful"

    users = list(users.items())
    ref.push().set({"id": users[0][1]["id"] + 1, "email": email, "password": create_hashed_password(password), "language" : "en"})
    return "successful"
...
def login(email, password)->str:
    ...
    if check_password(password, user[0][1]["password"]):
        return "successful"
...
def change_language(email, language) -> str:
    ...
    return f"Language set successfully"
    ...
    with conn:
        ...
        while True:
            ...
            if arr[0] == "new_user":
                ...
                print(f"{new_user(arr[1], arr[2]).encode('utf-8')}")
                continue
            ...
            if arr[0] == "get_all_users":
                conn.send(get_all_users().encode('utf-8'))
                continue
